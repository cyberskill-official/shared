...
§3.6 `delete(path, "purge")` is reserved for legal-erasure compliance (GDPR Art. 17 and equivalents). It MUST be gated by an explicit chat-turn approval (§16.2) AND a non-empty `reason`. The purge ledger row records the redacted content's hash but NOT its body; the _fact_ of purge is itself a ledger leaf and is not itself erasable.
