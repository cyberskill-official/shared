...
/**
 * Forces the `readPreference` query parameter of a MongoDB connection string to `primary`.
 *
 * @remarks
 * migrate-mongo reads its changelog collection through this URI. On a geo-distributed
 * replica set where the URI carries `readPreference=secondaryPreferred` (e.g. a follower
 * origin's connection string), a stale secondary read can make migrate-mongo believe an
 * already-applied migration is still pending and re-run it. Forcing `primary` here keeps
 * changelog reads consistent regardless of what read preference the application's own
 * connection string requests. A `readPreference=primary` param is harmless on
 * standalone/directConnection URIs — drivers ignore it there.
 *
 * Deliberately does **not** use `new URL()` — MongoDB multi-host URIs
 * (`mongodb://user:pass@h1:27017,h2:27017/db?x=y`) are not valid WHATWG URLs and break
 * `new URL()` parsing. Instead, only the query-string portion after the first `?` is
 * manipulated with `URLSearchParams`, leaving the scheme, credentials, and host list untouched.
 *
 * @param url - A `mongodb://` or `mongodb+srv://` connection string. Supports multi-host
 * comma lists, URI-encoded credentials, an existing `readPreference` param anywhere in the
 * query string, and URIs with no query string at all.
 * @returns The connection string with `readPreference=primary` set — replacing any existing
 * `readPreference` value, or appended if none was present.
 */
export function forcePrimaryReadPreference(url: string): string {
    const queryIndex = url.indexOf('?');

    if (queryIndex === -1) {
        return `${url}?readPreference=primary`;
    }
...
export const mongo: I_MongoUtils = {
    ...
    migrate: {
         ...
         *
         * @remarks
         * When `options.mongodb.url` is present, its `readPreference` query parameter is
         * forced to `primary` via {@link forcePrimaryReadPreference} before the config is
         * written. This guards against migrate-mongo reading its changelog collection from a
         * stale secondary (e.g. on a geo-distributed replica set with
         * `readPreference=secondaryPreferred` in the app's connection string), which could
         * otherwise cause an already-applied migration to look pending and be re-run.
         *
        ...
        setConfig: (options: Partial<migrate.config.Config> & { moduleSystem?: 'commonjs' | 'esm' }) => {
            const configToWrite = options.mongodb?.url
                ? {
                        ...options,
                        mongodb: {
                            ...options.mongodb,
                            url: forcePrimaryReadPreference(options.mongodb.url),
                        },
                    }
                : options;

            const optionsJS = `// This file is automatically generated by the Cyberskill CLI.\nmodule.exports = ${JSON.stringify(configToWrite, null, 4)}`;
    ...
    regexify<T>(filter?: T_QueryFilter<T>, fields?: (keyof T | string)[]): T_QueryFilter<T> {
        ...
        /** \n         * @security ReDoS mitigation — accented character alternation groups (e.g., `(a|à|á|...)`)\n         * create patterns with polynomial matching complexity when combined with `.*` anchors.\n         * Capping at 100 characters bounds the worst-case regex complexity to an acceptable level.\n         * For longer search terms, consider using MongoDB `$text` search indexes.\n         */
        const MAX_REGEX_INPUT_LENGTH = 100;
